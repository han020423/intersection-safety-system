...
class RoadV4Segmentor:
    def __init__(self, weights: str, input_size: Tuple[int, int] = (352, 640), device: str = "cpu") -> None:
        ...
        if unexpected:
            aux_keys = [k for k in unexpected if k.startswith("aux")]
            other_keys = [k for k in unexpected if not k.startswith("aux")]
            if aux_keys:
                print(f"[RoadV4Segmentor v3] ignored aux train-only keys: {len(aux_keys)}")
            if other_keys:
                print(f"[RoadV4Segmentor v3] unexpected keys: {len(other_keys)}")

        self.model.to(self.device).eval()
        print(f"[RoadV4Segmentor v3] lightweight BiSeNetV2 loaded {Path(weights).name}")
        print(f"[RoadV4Segmentor v3] input={self.input_size} device={device}")
