...
def _left_boundary_yellow_signal(seg_mask: np.ndarray,
    ...
    """
    ego lane 좌측 경계 주변의 yellow lane 비율을 계산한다.

    한국 도로에서 좌측 경계가 안정적으로 황색이면 중앙선/반대 방향 교통류와의
    경계일 가능성이 높다. 단, 일부 픽셀만 노란색인 경우를 피하려고 boundary
    주변 작은 patch들을 모아 yellow 비율과 샘플 수를 함께 확인한다.
    """
    if left_boundary is None or len(left_boundary) < 4:
        return False, 0.0, 0

    yellow = 0
    lane = 0
    radius = max(2, int(frame_w * 0.006))
    for x, y in np.asarray(left_boundary, dtype=np.int32).reshape(-1, 2):
    ...
    if lane < max(18, int(len(left_boundary) * 2.0)):
...
class EgoLanePipeline:
    ...
    def _estimate_lane_role(self,
        ...
        출력은 안전판단이 아니라 lane role 정보다. 확실하지 않으면 True/False 대신
        Unknown(None)을 유지하고, reason에 왜 모르는지 남긴다.
        """
        ...
        is_leftmost = left_idx == 0
        is_rightmost = right_idx == boundary_count - 1
        ...
        reason_parts = []
        if left_vehicle and is_leftmost:
            is_leftmost = False
            reason_parts.append("left_adjacent_vehicle")
        if right_vehicle and is_rightmost:
            is_rightmost = False
        ...
        if boundary_count == 2:
            ...
            if right_vehicle:
                is_rightmost = False
            elif left_yellow and previous_single_lane_streak >= 4:
                is_rightmost = True
                reason_parts.append("single_lane_stable")
            else:
                is_rightmost = None
...
def draw_ego_lane_overlay(vis: np.ndarray, result: EgoLaneResult, alpha: float = 0.24) -> None:
    ...
    if result.role is not None:
        ...
        yellow_text = f"LY:{role.left_yellow_ratio:.2f}" if role.left_boundary_yellow else "LY:-"
        role_text = (
            ...
            f"adjV:{adj} {yellow_text} conf:{role.confidence:.2f}"
        )
