...
MODEL       = "yolo26n.pt"      # 사전학습 가중치 (없으면 자동 다운로드)

EPOCHS      = 300               # 에포크 수 (더 나은 수렴을 위해 300으로 증가, patience 적용됨)
IMGSZ       = 640               # 입력 이미지 크기 (작은 객체가 많을 경우 800 등으로 증가시키면 성능이 향상됨)
BATCH       = 16                # 배치 크기 (GPU 메모리 부족 시 8로 감소)
PATIENCE    = 50                # Early stopping: 검증 mAP가 50 epoch 동안 개선 안되면 자동 종료
...
def main():
    ...
    results = model.train(
        ...
        # --- Advanced Hyperparameters for Performance ---
        optimizer = 'auto',     # 최적화 알고리즘 (성능과 안정성이 좋은 옵타마이저 자동 적용)
        cos_lr    = True,       # Cosine learning rate 스케줄러 적용 (안정적이고 높은 성능의 수렴 보장)
        close_mosaic = 15,      # 마지막 15 epoch에서는 mosaic을 꺼서 실제 이미지 스케일에 맞게 파인튜닝
        ...
        hsv_s     = 0.7,        # 채도 변형
        hsv_v     = 0.4,        # 명도 변형
        degrees   = 10.0,       # 회전 (다양한 각도 대응을 위해 10도로 증가)
        translate = 0.1,        # 이동
        scale     = 0.5,        # 크기 조절
        flipud    = 0.0,        # 상하반전 안함 (도로주행 특징 유지)
        fliplr    = 0.5,        # 50% 확률로 좌우반전
        mosaic    = 1.0,        # 모자이크 기법 (작은 객체 탐지에 매우 좋음)
        mixup     = 0.15,       # 객체가 겹치는 상황 학습을 위해 mixup 확률 소폭 증가
        erasing   = 0.4,        # Random Erasing (무작위 가림) - 신호등/표지판이 가려지는 환경에 대한 강건함 확보
    )
